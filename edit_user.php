		...
		if(ISSET($_POST['update'])){
            ...
            if (empty($file)){
                ...
                if ($password==$row1['password']){
                    $conn->query("UPDATE user SET username = '$username',role = '$role' WHERE id_user = '$id_user'")or die(mysql_error());
                    echo "<script> window.location='user.php' </script>";
                }else{
                    $conn->query("UPDATE user SET username = '$username',role = '$role',password = '$password1' WHERE id_user = '$id_user'")or die(mysql_error());
                    echo "<script> window.location='user.php' </script>";
                }
            }else{
                if (in_array($_FILES['image']['type'], ['image/jpeg', 'image/jpg', 'image/webp', 'image/png'])) {
                    ...
                    if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                        ...
                        //jika password sama
                        if ($password==$row1['password']){
                            $conn->query("UPDATE user SET username = '$username',role = '$role',foto = '$image_location' WHERE id_user = '$id_user'")or die(mysql_error());
                            echo "<script> window.location='user.php' </script>";
                        }else{
                            $conn->query("UPDATE user SET username = '$username',role = '$role',password = '$password1',foto = '$image_location' WHERE id_user = '$id_user'")or die(mysql_error());
                            echo "<script> window.location='user.php' </script>";
                        }
                    } else {
