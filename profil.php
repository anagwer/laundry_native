            ...
            <div class="card-body pt-3">
              ...
              <div class="tab-content pt-2">
                ...
                <?php 
                    require 'dbcon.php';
                    $id=$_SESSION['ID'];
                    $query = $conn->query("SELECT user.*,pelanggan.* FROM user join pelanggan on pelanggan.id_user=user.id_user WHERE user.id_user='$id'");
                  ...
                  <input name="id_user" type="hidden" class="form-control" id="id_user" value="<?php echo $row['id_user'];?>">
                    ...
                    <div class="row mb-3">
                      <label for="fullName" class="col-md-4 col-lg-3 col-form-label">No telp</label>
                      <div class="col-md-8 ">
                        <input name="no_telp" type="text" class="form-control" id="no_telp" value="<?php echo $row['no_telp'];?>">
                      </div>
                    </div>
                    <div class="row mb-3">
		...
		if(ISSET($_POST['update'])){
            ...
            if (empty($file)){
                ...
                if ($password==$row1['password']){
                    $conn->query("UPDATE user SET username = '$username' WHERE id_user = '$id_user'")or die(mysql_error());
                }else{
                    $conn->query("UPDATE user SET username = '$username',password = '$password1' WHERE id_user = '$id_user'")or die(mysql_error());
                }
                $conn->query("UPDATE pelanggan SET nama = '$nama',no_telp = '$no_telp',alamat = '$alamat',jns_kelamin = '$jns_kelamin' WHERE id_user = '$id_user'")or die(mysql_error());
                ...
                if (in_array($_FILES['image']['type'], ['image/jpeg', 'image/jpg', 'image/webp', 'image/png'])) {
                    ...
                    if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                        ...
                        //jika password sama
                        if ($password==$row1['password']){
                            $conn->query("UPDATE user SET username = '$username',foto = '$image_location' WHERE id_user = '$id_user'")or die(mysql_error());
                        }else{
                            $conn->query("UPDATE user SET username = '$username',password = '$password1',foto = '$image_location' WHERE id_user = '$id_user'")or die(mysql_error());
                        }
                        $conn->query("UPDATE pelanggan SET nama = '$nama',no_telp = '$no_telp',alamat = '$alamat',jns_kelamin = '$jns_kelamin' WHERE id_user = '$id_user'")or die(mysql_error());
                        echo "<script> window.location='profil.php' </script>";
                    } else {
